...
import { Row, Layout, Icon, Checkbox, Select } from 'antd';
import LayoutStyle from './_layout.scss';

const { Sider } = Layout;
const { Option } = Select;

export default class TotalLayout extends React.Component {
  state = {
    data: [],
    value: undefined,
    expand: false,
    ...
    return (
      <Fragment>
        <Row className={LayoutStyle.header}>
          <div className={LayoutStyle.title}>所有游戏</div>
          <div className={LayoutStyle.secondary}>浏览PPMall的所有产品</div>
        </Row>
            ...
            {this.state.expand ? null : (
              <Row className={LayoutStyle.rowNoLine}>
                <div onClick={this.toggleForm}>
                  <div>
                    <span style={{ color: '#f5f5f5' }}>查看全部</span> <Icon type="down" />
                  </div>
                </div>
              </Row>
            )}
